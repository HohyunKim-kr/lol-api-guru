...
const SummonerProfile = ({ summoner, championMasteries = [] }: SummonerProfileProps) => {
  return (
    <motion.div
        ...
        <div className="p-6 sm:p-8">
          <div className="flex flex-col sm:flex-row items-center sm:items-start gap-6">
            ...
            <div className="flex-1 text-center sm:text-left">
              ...
              >
                Level {summoner.summonerLevel} • Last updated {new Date(summoner.revisionDate).toLocaleDateString()}
              </motion.div>
              
              <Separator className="my-4" />
              
              <motion.div variants={staggerItem} className="flex flex-wrap justify-center sm:justify-start gap-4">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {championMasteries.slice(0, 5).map((mastery) => (
              ...
              >
                ...
                <p className="text-sm font-medium truncate">Champion {mastery.championId}</p>
                <p className="text-xs text-muted-foreground">{mastery.championPoints.toLocaleString()} pts</p>
              </div>
            ))}
          </div>
        </motion.div>
