...
const MatchHistoryItem = ({ match, summonerName }: { match: Match, summonerName: string }) => {
  ...
  const kdaRatio = ((participant.kills + participant.assists) / (participant.deaths || 1)).toFixed(2);
      ...
      <div className="p-4 sm:p-5">
        <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="relative">
              ...
              />
              <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-xs font-medium ${
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">{match.info.gameMode}</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-6">
            ...
            <div className="text-center">
              <p className="text-sm font-medium">{participant.totalDamageDealtToChampions.toLocaleString()}</p>
            ...
            <div className="text-center">
              <p className="text-sm font-medium">{participant.goldEarned.toLocaleString()}</p>
            ...
            <div className="text-center">
              <p className="text-sm font-medium">{gameDate.toLocaleDateString()}</p>
              <p className="text-xs text-muted-foreground">{gameDuration} min</p>
            </div>
          </div>
        ...
        <div className="mt-4 flex flex-wrap gap-1">
          {[
            ...
            participant.item5,
            participant.item6,
          ].filter(Boolean).map((itemId) => (
            <div key={itemId} className="w-8 h-8 bg-black/30 rounded overflow-hidden">
              <img
    ...
    >
      ...
      {matches.length === 0 ? (
        <div className="glass-card rounded-xl p-8 text-center">
          <p className="text-muted-foreground">No matches found for this summoner.</p>
        </div>
      ) : (
        matches.map((match) => (
          <MatchHistoryItem 
